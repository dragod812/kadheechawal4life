...
for group in groups:
    ...
    if group == 'Sidharth family':
        ...
        premium = sum(r for _, r, _ in kalyani_allocations)
        extras = sum(e for _, _, e in kalyani_allocations)
    else:
        premium, extras = 2, 0
...
para('Planning basis: 26 guests on 25 February (two per suite, three per Premium Room); the full guest list on 26–27 February. Individual stays and sharing partners to be confirmed.')
...
para('Calculation assumes suites for Sidharth fam (3), Mamu (4), Mausi (4) and three of Abhishek/Ritesh’s five guests. The remaining two share a Premium Room; Khuku Badama’s two share another; Latika Aunty has one alone. Couples stay together. Friend singles share up to three per room within their group; Sourav’s Friends and Saurav’s team use twin sharing. Kalyani family uses the supplied room allocation. Guruji and the photographer each have one room alone.')
html.append('</details>')
